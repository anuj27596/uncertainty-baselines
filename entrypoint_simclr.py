...
('baselines.isic.jax_pretrain_simclr', {
			'output_dir': 'gs://ue-usrl-anuj/karm/outputs/isic/head_neck/vit_simclr/crop_<_crop>/<_seed>',
			'config': 'configs/isic/vit_simclr_pretrain.py',
			'config.seed': '<_seed>',
			'config.pp_train' : 'isic_preprocess(512)|simclr_aug(<_crop>)',
   			'config.pp_eval' : 'isic_preprocess(512)|simclr_aug(<_crop>)'
	},  
  {	'_seed': range(1),
	} ,
 {	'_crop': [0.36, 0.64, 0.81],
	}
 ),
]
