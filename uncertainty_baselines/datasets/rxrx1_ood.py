...
_NUM_EXAMPLES = 81224
_NUM_TRAINING_EXAMPLES = 40612
_NUM_CLASSES = 1139 # Karm
...
class Rxrx1OodConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(Rxrx1OodConfig, self).__init__(
...
class Rxrx1Ood(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "rxrx1_ood", "images"),
                "split": split,
                "csv_path": os.path.join(path, "rxrx1_ood", "metadata.csv"),
            })
        for split in splits
    ]
  ...
  def _generate_examples(self,
    ...
    for idx, (path, label) in data.iterrows(): # Karm
      ...
      record = {
          ...
          "label": tf.one_hot(label, _NUM_CLASSES).numpy() # Karm
      }
      yield path, record
...
class Rxrx1OodDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "rxrx1_ood/processed",
    ...
    print(f"Using Rxrx1Ood builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(Rxrx1OodDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building Rxrx1 OOD dataset")
