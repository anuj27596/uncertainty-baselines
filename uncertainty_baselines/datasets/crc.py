...
class CrcConfig(tfds.core.BuilderConfig):
...
class Crc(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "crc", "NCT-CRC-HE-100K"),
                "split": split,
                "csv_path": os.path.join(path,"crc", "NCT-CRC-HE-100K", "labels.csv"),
            })
        for split in splits
    ]
  ...
  def _generate_examples(self,
    ...
    if csv_path:
      ...
      if "tum" in self.builder_config.name:
        positive_class = "TUM"
      df = df[df.label.isin(['NORM', positive_class])]
    #   df['label'] = df['label'].apply(lambda x: 0 if x=='NORM' else 1)
      ...
      if 'swap' in self.builder_config.name:
        ...
        test_df, val_df =  train_test_split(test_df, test_size=0.5, stratify=test_df['label'], random_state=0)
        if split == "train":
          data = train_df
        ...
        if split == "validation":
          ...
          data = test_df
          # print(f" test target sum - {sum(test_df['label'])}")
    ...
    for idx, (path, label) in data.iterrows(): # Karm
      image_filepath = os.path.join(images_dir_path, label, path)
      label = 0 if label=='NORM' else 1
      record = {
          "name": path,
...
class CrcDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "crc/processed",
    ...
    print(f"Using Crc builder config {builder_config}.")
    # import pdb; pdb.set_trace()
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(CrcDataset, self).__init__(
