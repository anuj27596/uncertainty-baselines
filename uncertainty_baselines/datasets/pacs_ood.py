...
with the larger Kaggle/EyePACSOod Diabetic Retinopathy Detection dataset.
...
class PacsOodConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(PacsOodConfig, self).__init__(
...
class PacsOod(tfds.core.GeneratorBasedBuilder):
...
class PacsOodDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "pacs_ood/processed_sketch",
    ...
    print(f"Using PacsOod builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(PacsOodDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building PacsOod OOD dataset")
