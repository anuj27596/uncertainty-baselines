...
class Rxrx1IdConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(Rxrx1IdConfig, self).__init__(
...
class Rxrx1Id(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "rxrx1_id", "images"),
                "split": split,
                "csv_path": os.path.join(path, "rxrx1_id", "metadata.csv"),
            })
        for split in splits
    ]
...
class Rxrx1IdDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "rxrx1_id/processed",
    ...
    print(f"Using Rxrx1Id builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(Rxrx1IdDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building Rxrx1 ID dataset")
