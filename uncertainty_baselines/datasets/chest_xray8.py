...
class ChestXray8Config(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(ChestXray8Config, self).__init__(
...
class ChestXray8(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "chest_xray8", "images"),
                "split": split,
                "csv_path": os.path.join(path, "chest_xray8", "metadata.csv"),
            })
        for split in splits
    ]
...
class ChestXray8Dataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "chest_xray8/processed",
    ...
    print(f"Using ChestXray8 builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(ChestXray8Dataset, self).__init__(
        ...
        cache=cache)
    print(f"Building ChestXray8 OOD dataset")
