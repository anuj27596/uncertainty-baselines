...
class InbreastConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(InbreastConfig, self).__init__(
...
class Inbreast(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path,"inbreast", "images"),
                "split": split,
                "csv_path": os.path.join(path,"inbreast", "data.csv"),
            })
        for split in splits
    ]
...
class InbreastDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "isic_id/processed",
    ...
    print(f"Using Inbreast builder config {builder_config}.")
    # import pdb; pdb.set_trace()
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(InbreastDataset, self).__init__(
