...
class ZhangPneumoniaConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(ZhangPneumoniaConfig, self).__init__(
...
class ZhangPneumonia(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "zhang_pneumonia", "images"),
                "split": split,
                "csv_path": os.path.join(path, "zhang_pneumonia", "metadata.csv"),
            })
        for split in splits
    ]
  ...
  def _generate_examples(self,
    ...
    Args:
      ...
      split: bool, use validation set (20%) else use test (80%).
    ...
    if csv_path:
      ...
      data = [(idx, int(pneumonia))
              for idx, pneumonia, s in df[['id', 'label', 'split']].iloc
              if s in split]
...
class ZhangPneumoniaDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "zhang_pneumonia/processed",
    ...
    print(f"Using ZhangPneumonia builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(ZhangPneumoniaDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building ZhangPneumonia dataset")
