...
from uncertainty_baselines.datasets.processing_pneumonia import _pneumonia_processing, _resize_image_if_necessary, _resize_image_if_necessary_pcam
...
class PcamConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(PcamConfig, self).__init__(
...
class Pcam(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      ...
      PcamConfig(
          ...
          target_pixels=224),
      PcamConfig(
          name="processed_onehot",
          description=_BTGRAHAM_DESCRIPTION_PATTERN.format(300),
          target_pixels=224),
      PcamConfig(
          name="processed_onehot_tum",
          description=_BTGRAHAM_DESCRIPTION_PATTERN.format(300),
          target_pixels=224),
      PcamConfig(
          name="processed_onehot_tum_swap",
          description=_BTGRAHAM_DESCRIPTION_PATTERN.format(300),
          target_pixels=224),
  ]
  ...
  def _info(self):
    ...
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            # "image": tfds.features.Tensor(shape=(96,96,3), dtype=tf.dtypes.float32),
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path, "pcam", f"camelyonpatch_level_2_split_{split}_x.h5"),
                "split": split,
                "csv_path": os.path.join(path,"pcam", f"camelyonpatch_level_2_split_{split}_y.h5"),
            })
        for split in splits
    ]
  ...
  def _generate_examples(self,
    ...
    #   df['label'] = df['label'].apply(lambda x: 0 if x=='NORM' else 1)
      
      with h5py.File(images_dir_path, "r") as f:
        print(f"Keys: {f.keys()}")
        X = np.array(f['x'])
    ...
    for i in range(N): # Karm
      img_arr, label = X[i], int(y[i])
      path = f"{i}_{images_dir_path}"
      record = {
          "name": path,
          "image": _resize_image_if_necessary_pcam(img_arr, target_pixels=self.builder_config.target_pixels),
...
class PcamDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "pcam/processed",
    ...
    print(f"Using Pcam builder config {builder_config}.")
