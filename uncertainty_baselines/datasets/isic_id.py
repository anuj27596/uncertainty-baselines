...
class IsicIdConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(IsicIdConfig, self).__init__(
...
class IsicId(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        tfds.core.SplitGenerator(
            name=split,
            gen_kwargs={
                "images_dir_path": os.path.join(path,"isic_id", "train"),
                "split": split,
                "csv_path": os.path.join(path,"isic_id","ISIC_2020_Training_GroundTruth.csv"),
            })
        for split in splits
    ]
...
class IsicIdDataset(base.BaseDataset):
  ...
  def __init__(self,
    ...
    print(f"Using IsicId builder config {builder_config}.")
    # import pdb; pdb.set_trace()
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(IsicIdDataset, self).__init__(
