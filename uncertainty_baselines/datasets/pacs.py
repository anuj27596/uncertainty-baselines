...
class Pacs(tfds.core.GeneratorBasedBuilder):
  ...
  def _generate_examples(self,
    ...
    for i, (path, label) in df.iterrows():
        ...
        "label" : tf.one_hot(int(row.label),_NUM_CLASSES).numpy()
        }
        yield path, record
...
class PacsDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "pacs/processed_sketch",
    ...
    print(f"Using Pacs builder config {builder_config}.")
