...
with the larger Kaggle/EyePACSId Diabetic Retinopathy Detection dataset.
...
_NUM_CLASSES = 7 # Karm
_DATA_DOMAINS = ["art_painting", "cartoon", "photo", "sketch"]
...
class PacsIdConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(PacsIdConfig, self).__init__(
...
class PacsId(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      ...
      PacsIdConfig(
          name="processed_art_painting",
          description=_BTGRAHAM_DESCRIPTION_PATTERN.format(300),
          target_pixels=300),
        PacsIdConfig(
        name="processed_cartoon",
        description=_BTGRAHAM_DESCRIPTION_PATTERN.format(300),
        target_pixels=300),
            PacsIdConfig(
  ...
  def _info(self):
    ...
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            "label": tfds.features.Tensor(shape = (_NUM_CLASSES, ), dtype=tf.dtypes.float32) # Karm
        }),
  ...
  def _generate_examples(self,
    ...
    for domain in domains:
      split_file = os.path.join(images_dir_path, "split", f"id_{domain}_{split}.txt")
      print(f"\n *********** In Domain: Reading: {split_file} ***********") 
    ...
    for i, (path, label) in df.iterrows():
        record = {
        "name" : path,
        "image" : self._process_image(os.path.join(images_dir_path,"data",path)),
        "label" : tf.one_hot(int(label),_NUM_CLASSES).numpy()
        }
        yield path, record
...
class PacsIdDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "pacs_id/processed_sketch",
    ...
    print(f"Using PacsId builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(PacsIdDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building PacsId OOD dataset")
