...
class CancerCam16EmbdConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, target_pixels=None, blur_constant=None, **kwargs):
    ...
    """
    super(CancerCam16EmbdConfig, self).__init__(
...
class CancerCam16Embd(tfds.core.GeneratorBasedBuilder):
  ...
  def _generate_examples(self,
    ...
    path = images_dir_path # "/data3/home/karmpatel/dsmil-wsi/datasets/Camelyon16/"
    df = pd.read_csv(csv_path)
    df["wsi"] = df.iloc[:,0].apply(lambda x: x.split("/")[-1][:-4])

    df_test = df[df.iloc[:,0].apply(lambda x: "test" in x)]
    df_train = df[~df.iloc[:,0].apply(lambda x: "test" in x)]
    if split == "train":
        df_curr = df_train
    elif split == "test":
        df_curr = df_test
    ...
    for i, row in df_curr.iterrows():
        df_emb = pd.read_csv(path + 'embs/' + f'{row["wsi"]}.csv')
        record = {
            "wsi_name": row["wsi"],
...
class CancerCam16EmbdDataset(base.BaseDataset):
  """Kaggle APTOS 2019 Blindness Detection dataset builder class."""

  def __init__(self,
               split: str,
               builder_config: str = "cancer_cam16_embed/processed",
    ...
    print(f"Using CancerCam16Embd builder config {builder_config}.")
    dataset_builder = tfds.builder(builder_config, data_dir=data_dir)
    super(CancerCam16EmbdDataset, self).__init__(
        ...
        cache=cache)
    print(f"Building CancerCam16Embd OOD dataset")
