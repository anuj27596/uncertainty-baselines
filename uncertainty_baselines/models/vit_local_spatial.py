...
class VisionTransformerLocalSpatial(nn.Module):
  ...
  def __call__(self, inputs, *, train):
    ...
    local_spatial_proj = ProjectionHead(
        dim=c,
        name='local_spatial_head')(
            out['transformed'][:, 1:],  # discard cls
            deterministic=not train)
    out['local_spatial_proj'] = local_spatial_proj

    return logits, out


def vision_transformer_local_spatial(num_classes: int,
                                     patches: Any,
                                     transformer: Any,
                                     hidden_size: int,
                                     representation_size: Optional[int] = None,
  ...
  # cleaned up.
  return VisionTransformerLocalSpatial(
