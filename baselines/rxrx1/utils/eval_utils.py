...
def compute_loss_and_accuracy_arrs(results):
  ...
  """
  results = compute_log_loss_arr(results, labels=np.array(range(1139)))
  y_pred, y_true = results['y_pred'], results['y_true']
  # results['accuracy_arr'] = y_true == (y_pred > 0.5)
  results['accuracy_arr'] = y_true == np.argmax(y_pred, axis=-1) # karm
  return results
...
def compute_dataset_eval_metrics(
  ...
  # Standard predictive metrics
  labels=np.array(range(1139))
  eval_metrics[f'{dataset_key}/negative_log_likelihood'] = log_loss(
      y_pred=y_pred, y_true=y_true, labels=labels) # karm
  try:
    eval_metrics[f'{dataset_key}/auroc'] = roc_auc_score(
        y_true=y_true, y_score=y_pred, labels=labels)
    eval_metrics[f'{dataset_key}/auroc_weighted'] = roc_auc_score(
        y_true=y_true, y_score=y_pred, labels=labels, average="weighted")
  ...
  #     y_true=y_true, probas_pred=y_pred)
  # eval_metrics[f'{dataset_key}/auprc'] = auc(recall, precision)
  eval_metrics[f'{dataset_key}/auprc'] = None # karm
  eval_metrics[f'{dataset_key}/accuracy'] = (
      accuracy_score(y_true=y_true, y_pred= np.argmax(y_pred, axis=-1))) # karm

  # Uncertainty metrics
  # ece = rm.metrics.ExpectedCalibrationError(num_bins=ece_num_bins)
  # ece.add_batch(y_pred, label=y_true)
  # eval_metrics[f'{dataset_key}/ece'] = ece.result()['ece']
  eval_metrics[f'{dataset_key}/ece'] = None # karm

  if False and compute_open_set_recognition: # karm: Turned off calculating, giving error for multiclass
    is_ood = results['is_ood']
  ...
  if False and compute_retention_auc: # karm: turned off calculating, giving errors in multi-class
    assert 'accuracy_arr' in results
    assert 'nll_arr' in results
    eval_metrics[f'{dataset_key}/retention_accuracy_auc'] = np.mean(
