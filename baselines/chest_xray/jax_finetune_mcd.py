...
def main(argv):
  ...
  def evaluation_fn(params, images, labels, rng):
    ...
    probs = jnp.mean(jax.nn.sigmoid(logits), axis=-1)
    losses = -jnp.sum(labels * jnp.log(probs) + (1. - labels) * jnp.log(1. - probs), axis=-1)
    ...
    ncorrect = 0 # jax.lax.psum(top1_correct, axis_name='batch')
    n = batch_size_eval
