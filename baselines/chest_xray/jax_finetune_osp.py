  ...
  for step, train_batch, lr_repl in zip(
    ...
    with jax.profiler.TraceAnnotation('train_step', step_num=step, _r=1):
      if not config.get('only_eval', False):
        ...
        opt_repl, loss_value, train_loop_rngs, extra_measurements = update_fn(
            ...
            lr_repl,
            train_batch['image'],
            jnp.concatenate([1 - train_batch['labels'], train_batch['labels']], axis=-1),
            rng=train_loop_rngs)
    ...
    if train_utils.itstime(step, config.log_eval_steps, total_steps):
      ...
      for eval_name, (eval_iter, eval_steps) in eval_iter_splits.items():
        ...
        for _, batch in zip(range(eval_steps), eval_iter):
          batch_ncorrect, batch_losses, batch_n, batch_metric_args = (  # pylint: disable=unused-variable
              evaluation_fn(
                  opt_repl.target,
                  batch['image'],
                  jnp.concatenate([1 - batch['labels'], batch['labels']], axis=-1)))
          ...
          logits = logits.reshape(*logits.shape[:-1], 2, config.num_classes // 2)
          probs = jax.nn.softmax(logits, axis=-2)[..., 1, :]  # EDIT(anuj)

          labels = np.array(labels[0])  # EDIT(anuj)
          labels = labels.reshape(*labels.shape[:-1], 2, config.num_classes // 2)
          labels = labels[..., 1, :]  # EDIT(anuj)

          probs = np.reshape(probs, (probs.shape[0] * probs.shape[1], -1))
          logits = np.reshape(logits, (logits.shape[0] * logits.shape[1], *logits.shape[2:]))
        ...
        results_arrs['y_pred_entropy'] = vit_utils.entropy_from_logits(results_arrs['logits'][:, 1] - results_arrs['logits'][:, 0])  # EDIT(anuj)
        if config.only_eval:  # EDIT(anuj)
