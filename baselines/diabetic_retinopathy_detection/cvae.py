...
flags.DEFINE_string(
    'output_dir', '/tmp/diabetic_retinopathy_detection/cvae',
...
def main(argv):
  ...
  with strategy.scope():
    logging.info('Building Keras SSVAE-M2 model.')
    model = None
  ...
  gen_loss_fn = utils.get_cvae_generative_loss_fn(FLAGS.cvae_coeff, FLAGS.cvae_dims)
  ...
  uncertainty_estimator_fn = utils.get_uncertainty_estimator( # TODO(anuj): checkout
      'cvae', use_ensemble=False, use_tf=True)
