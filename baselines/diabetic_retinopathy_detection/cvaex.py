...
flags.DEFINE_string(
    'output_dir', '/tmp/diabetic_retinopathy_detection/cvaex',
...
flags.DEFINE_float('cvae_dims', 2048, 'CVAE coefficient') # anuj
...
def main(argv):
  ...
  with strategy.scope():
    logging.info('Building Keras CVAEx model.')
    model = None
    ...
    metrics = utils.get_diabetic_retinopathy_base_metrics( # TODO(anuj): checkout later
        use_tpu=use_tpu,
  ...
  if not use_tpu:
    metrics.update(
        utils.get_diabetic_retinopathy_cpu_metrics( # TODO(anuj): checkout later
            available_splits=available_splits,
            use_validation=FLAGS.use_validation))
  ...
  gen_loss_fn = utils.get_cvaex_generative_loss_fn()

  # * Prepare for Evaluation *

  # Get the wrapper function which will produce uncertainty estimates for
  # our choice of method and Y/N ensembling.
  uncertainty_estimator_fn = utils.get_uncertainty_estimator( # TODO(anuj): checkout
      'cvaex', use_ensemble=False, use_tf=True)

  # Wrap our estimator to predict probabilities (apply sigmoid on logits)
  # eval_estimator = utils.wrap_retinopathy_estimator( # TODO(anuj): checkout
  #     model, use_mixed_precision=FLAGS.use_bfloat16, numpy_outputs=False)
  eval_estimator = model
  ...
  def train_step(iterator):
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        outputs = model(dict(image=images, label=labels), training=True)
        ...
        nl_elbo = gen_loss_fn(inputs, outputs)

        l2_loss = sum(model.losses)
        loss = nl_elbo + (FLAGS.l2 * l2_loss)
      ...
      probs = tf.squeeze(tf.nn.sigmoid(tf.reduce_mean(outputs['mean'], axis = tuple(range(1, len(outputs['mean'].shape))))))
