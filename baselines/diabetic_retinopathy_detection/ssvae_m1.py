...
flags.DEFINE_string(
    'output_dir', '/tmp/diabetic_retinopathy_detection/ssvae_m1',
...
def main(argv):
  ...
  with strategy.scope():
    logging.info('Building Keras SSVAE-M1 model.')
    model = None
  ...
  gen_loss_fn = utils.m1_generative_loss_fn
  ...
  uncertainty_estimator_fn = utils.get_uncertainty_estimator( # TODO(anuj): checkout
      'ssvae_m1', use_ensemble=False, use_tf=True)
  ...
  def train_step(iterator):
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        nl_elbo = gen_loss_fn(images, outputs)

        l2_loss = sum(model.losses)
        loss = negative_log_likelihood + nl_elbo + (FLAGS.l2 * l2_loss)
      ...
      metrics['train/negative_log_likelihood'].update_state(
          negative_log_likelihood) # TODO(anuj): add metrics
  ...
  for layer in model.layers: # anuj
    layer.trainable = not layer.name.startswith('clf')

  for epoch in range(initial_epoch, FLAGS.train_epochs):

    if epoch > FLAGS.train_epochs * 2 / 3:
      for layer in model.layers: # anuj
        layer.trainable = layer.name.startswith('clf')
