...
# import checkpoint_utils  # local file import from baselines.histopathology  # EDIT(anuj)
# import input_utils  # local file import from baselines.histopathology  # EDIT(anuj)
# import preprocess_utils  # local file import from baselines.histopathology  # EDIT(anuj)
# import train_utils  # local file import from baselines.histopathology  # EDIT(anuj)
# from utils import results_storage_utils  # EDIT(anuj)
# from utils import vit_utils  # EDIT(anuj)
import baselines.histopathology.checkpoint_utils as checkpoint_utils  # EDIT(anuj)
import baselines.histopathology.input_utils as input_utils  # EDIT(anuj)
import baselines.histopathology.preprocess_utils as preprocess_utils  # EDIT(anuj)
import baselines.histopathology.train_utils as train_utils  # EDIT(anuj)
from baselines.histopathology.utils import results_storage_utils  # EDIT(anuj)
from baselines.histopathology.utils import vit_utils  # EDIT(anuj)
import wandb
...
def main(argv):
  ...
  train_base_dataset = ub.datasets.get(
      ...
      builder_config=f"{dataset_names['in_domain_dataset']}/{config.builder_config}")
  ...
  train_ood_base_dataset = ub.datasets.get(
      ...
      data_dir=config.get('data_dir'),
      builder_config=f"{dataset_names['ood_dataset']}/{config.builder_config}")
  ...
  model_dict = vit_utils.initialize_model('mae', config)  # EDIT(anuj)
  ...
  def init(rng):
    ...
    dummy_mask = jnp.zeros((local_batch_size, int(config.mask_rate * (h // p) * (w // p))), jnp.int32)
    params = flax.core.unfreeze(model.init(rng, dummy_input, dummy_mask,
                                           train=False))['params']
  ...
  def evaluation_fn(params, images, labels, mae_mask):
    logits, out = model.apply(
        {'params': flax.core.freeze(params)}, images, mae_mask, train=False)
    losses = train_utils.mae_loss(inputs=images, pred=out['mae_pred'], mask=mae_mask, patchsize=config.model.patches.size[0], reduction=False)  # EDIT(anuj)
  ...
  def update_fn(opt, lr, images, labels, mae_mask, rng):
    """Update step."""
    measurements = {}
    ...
    def loss_fn(params, images, labels, mae_mask):
      logits, out = model.apply(  # EDIT(anuj)
          {'params': flax.core.freeze(params)}, images, mae_mask,
          train=True, rngs={'dropout': rng_model_local})
      return train_utils.mae_loss(inputs=images, pred=out['mae_pred'], mask=mae_mask, patchsize=config.model.patches.size[0])  # EDIT(anuj)
    ...
    l, g = jax.value_and_grad(loss_fn)(opt.target, images, labels, mae_mask)
    ...
    with jax.profiler.TraceAnnotation('train_step', step_num=step, _r=1):
      if not config.get('only_eval', False):
        ...
        opt_repl, loss_value, train_loop_rngs, extra_measurements = update_fn(
            ...
            train_batch['labels'],
            jnp.concatenate([train_batch['mae_mask'], train_ood_batch['mae_mask']], axis=1),
            rng=train_loop_rngs)
      ...
      for eval_name, (eval_iter, eval_steps) in eval_iter_splits.items():
        ...
        for _, batch in zip(range(eval_steps), eval_iter):
          batch_ncorrect, batch_losses, batch_n, batch_metric_args = (  # pylint: disable=unused-variable
              evaluation_fn(
                  opt_repl.target, batch['image'], batch['labels'], batch['mae_mask']))
