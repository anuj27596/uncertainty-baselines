...
def main(argv):
  ...
  train_base_dataset = ub.datasets.get(
      ...
      builder_config=f"{dataset_names['in_domain_dataset']}/{config.builder_config}") # Karm
  ...
  model_dict = vit_utils.initialize_model('osp', config)
  ...
  def init(rng):
    ...
    dummy_labels = jnp.zeros((local_batch_size, config.num_classes), jnp.float32)
    params = flax.core.unfreeze(model.init(rng, dummy_input, dummy_labels,
                                           train=False))['params']
  ...
  def evaluation_fn(params, images, labels):
    logits, out = model.apply(
        {'params': flax.core.freeze(params)}, images, labels, train=False)
  ...
  def update_fn(opt, lr, images, labels, rng):
    ...
    def loss_fn(params, images, labels):
      logits, out = model.apply(
          {'params': flax.core.freeze(params)}, images, labels,
          train=True, rngs={'dropout': rng_model_local})
      return out['lagrangian']
