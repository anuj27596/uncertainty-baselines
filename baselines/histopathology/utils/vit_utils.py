...
# import input_utils  # local file import from baselines.histopathology
import baselines.histopathology.input_utils as input_utils  # anuj
...
def get_dataset_and_split_names(dist_shift):
  ...
  dataset_names = {}
  split_names = {}
  if "swap" in dist_shift:
    dataset_names['in_domain_dataset'] = 'crc'
    dataset_names['ood_dataset'] = 'pcam'
  else:
    dataset_names['in_domain_dataset'] = 'pcam'
    dataset_names['ood_dataset'] = 'crc'
...
def initialize_mae_model(config):  # EDIT(anuj)
  logging.info('config.model = %s', config.get('model'))
  model = ub.models.vision_transformer_mae(
      num_classes=config.num_classes, **config.get('model', {}))
  return {
      'model': model
  }
...
def init_evaluation_datasets(use_train,  # EDIT(anuj)
  ...
  datasets = {}
  if "swap" in config.builder_config:
    id_dataset, ood_dataset = 'crc', 'pcam'
  else:
    id_dataset, ood_dataset = 'pcam', 'crc'
  ...
  if use_train:  # EDIT(anuj)
    datasets['train'] = get_dataset(
        ...
        builder_config =f'{id_dataset}/{config.builder_config}')
  if use_validation:
    datasets['in_domain_validation'] = get_dataset(
        dataset_name=dataset_names['in_domain_dataset'],
        split_name=split_names['in_domain_validation_split'], 
        builder_config =f'{id_dataset}/{config.builder_config}')
        # builder_config ='histopathology/{config.builder_config}-left') # Karm: This will not do anything
    datasets['ood_validation'] = get_dataset(
        dataset_name=dataset_names['ood_dataset'],
        split_name=split_names['ood_validation_split'],
        builder_config = f"{ood_dataset}/{config.builder_config}")
        # builder_config = "histopathology/{config.builder_config}-left") # Karm
  if use_test:
    ...
    datasets['ood_test'] = get_dataset(
        ...
        builder_config = f"{ood_dataset}/{config.builder_config}")
        # builder_config = "histopathology/{config.builder_config}-left")
