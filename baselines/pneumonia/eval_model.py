...
flags.DEFINE_string(
    'preproc_builder_config', 'processed', 'preproc_builder_config')
FLAGS = flags.FLAGS
    ...
    with strategy_scope:
      ...
      if 'fsvi' in model_type:
        ...
        if use_ensemble or single_model_multi_train_seeds:
          ...
          estimator = [
              utils.wrap_pneumonia_estimator(
                  loaded_model,
          ...
          estimator = utils.wrap_pneumonia_estimator(
              model,
              use_mixed_precision=FLAGS.use_bfloat16,
              return_logits=True,  # anuj
              numpy_outputs=not FLAGS.use_distribution_strategy)
...
def construct_input_and_output_dir(model_type, tuning_domain,
                                   single_model_multi_train_seeds, n_samples, k,
                                   FLAGS):
  output_suffix = 'single' if single_model_multi_train_seeds else 'ensemble'
  if FLAGS.chkpt_bucket.startswith('ue-usrl-anuj/checkpoints'):  # anuj
    checkpoint_dir = (f'gs://{FLAGS.chkpt_bucket}/'
                      f'{model_type}_k{k}_{tuning_domain}')
    output_dir = (
        f'gs://{FLAGS.output_bucket}/'
