...
PNEUMONIA_MODEL_TO_DECOMPOSED_UNCERTAINTY_ESTIMATOR = {
    ('deterministic', False):
...
PNEUMONIA_MODEL_TO_TF_DECOMPOSED_UNCERTAINTY_ESTIMATOR = {
    ('deterministic', False):
...
def wrap_pneumonia_estimator(estimator,
                               use_mixed_precision,
                               return_logits=False,
                               numpy_outputs=True):
  ...
  # embedder = tf.keras.Model(estimator.input, estimator.layers[-1].input) if return_logits else None  # anuj
  ...
  def estimator_wrapper(inputs, training, estimator):
    ...
    if numpy_outputs and return_logits:
      return probs.numpy(), logits.numpy() # , embedder(inputs, training=training).numpy()  # anuj
    elif return_logits:
      return probs, logits # , embedder(inputs, training=training)  # anuj
    elif numpy_outputs:
      return probs.numpy()
    else:
      return probs
...
def get_uncertainty_estimator(model_type, use_ensemble, use_tf):
  ...
  try:
    if use_tf:
      ...
      uncertainty_estimator_fn = (
          PNEUMONIA_MODEL_TO_DECOMPOSED_UNCERTAINTY_ESTIMATOR[(model_type,
                                                                 use_ensemble)])
  except KeyError:
