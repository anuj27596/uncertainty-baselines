...
import baselines.pneumonia.utils as utils  # anuj
import wandb
...
flags.DEFINE_string(
    'output_dir', '/tmp/pneumonia/dropout',
...
flags.DEFINE_string('preproc_builder_config', 'processed', (
...
flags.DEFINE_integer(
    ...
    'and evaluation.')
flags.DEFINE_integer(
    'checkpoint_interval', 10, 'Number of epochs between saving checkpoints. '
...
def main(argv):
  ...
  if class_reweight_mode == 'constant':
    class_weights = utils.get_pneumonia_class_balance_weights()
  else:
    class_weights = None
  ...
  with strategy.scope():
    ...
    metrics = utils.get_pneumonia_base_metrics(
        use_tpu=use_tpu,
  ...
  if not use_tpu:
    metrics.update(
        utils.get_pneumonia_cpu_metrics(
            available_splits=available_splits,
            use_validation=FLAGS.use_validation))
  ...
  loss_fn = utils.get_pneumonia_loss_fn(
  ...
  eval_estimator = utils.wrap_pneumonia_estimator(
