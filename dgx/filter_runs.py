...
with open("txt_files/isic_dan_iw_done_dgx.txt") as fp:
    done = fp.readlines()
...
for module, static_args, *iter_args in run_settings:
    ...
    for vals in itertools.product(*[zip(*d.values()) for d in iter_args]):
        ...
        run_args = run['args']
        run_string = f"lr_{run_args['config.lr.base']}/dlc_{run_args['config.dp_loss_coeff']}/nl_{run_args['config.model.domain_predictor.num_layers']}/hdim_{run_args['config.model.domain_predictor.hid_dim']} *"
        if run_string not in done:
            left.append(run_id)
        run_id+=1

print(f"left runs: {left}")
