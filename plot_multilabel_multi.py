...
def process_func(m, d, seed, SR, semaphore):
    ...
    if m.startswith('rb'):
        ...
        resdir = os.path.join(model_path[m], str(seed), d, 'eval_results_1250')
        print(resdir)
    ...
    for r_idx, r in enumerate(REFRANGE):
        ...
        if SR:
            ...
            y_true_selected, y_hat_selected, y_pred_selected = np.argmax(y, axis=1)[ids], np.argmax(p, axis=1)[ids], np.max(p,axis=1)[ids]
