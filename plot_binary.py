...
def process_func(m, d, seed, SR, semaphore):
	semaphore.acquire()
	...
	if m.startswith('rb'):
		...
		resdir = os.path.join(model_path[m], str(seed), d, 'eval_results_1')
		y = np.load(os.path.join(resdir, 'y_true.npy'))[..., None]
		if m == 'vit_osp':
			l = np.load(os.path.join(resdir, 'logits.npy')).reshape(-1, 2)
			pred_class = l.argmax(axis = 1)
			pred_certainty = l.max(axis = 1)
			p = 0.5 + (pred_class - 0.5) * ecdf(pred_certainty)
			p = p[..., None]
		else:
			p = np.load(os.path.join(resdir, 'y_pred.npy'))[..., None]
		# e = np.load(os.path.join(resdir, 'pre_logits.npy'))
		# e = e.reshape(-1, e.shape[-1])

	metrics = {
		score: np.zeros((NUM_CLASSES, REFRANGE.size))
		for score in ('AvgPrec', 'Accuracy', 'AUROC')
	}

	for class_id in range(NUM_CLASSES):
		if SR:
			order = split_referral_order(p[:, class_id])
		else:
			order = np.abs(p[:, class_id] - 0.5).argsort()

		for r_idx, r in enumerate(REFRANGE):
			ids = order[int(r * order.size):]
			metrics['Accuracy'][class_id, r_idx] = skm.accuracy_score(
					y[ids, class_id], p[ids, class_id].round())
	...
	for m, d, SR in itertools.product(models, datasets, (False, True)):
		for seed in seeds.get(m, range(6)):
			if os.path.exists(f'dump/{m}-{d}-{seed}-{SR}-AvgPrec.npy'):
				continue
			proc_obj = multiprocessing.Process(
				target = process_func,
				args = (m, d, seed, SR, semaphore),
			)
			proc_obj_list.append(proc_obj)
			proc_obj.start()
	...
	for d in datasets:
		...
		for idx1, p in enumerate(metrics):
			...
			# for SR, m in itertools.product((False, ), models):
				...
				# 	continue
				mean = np.mean(metrics[p][m, d, SR], axis = (0, 1))
			...
			for SR, m in itertools.product((False, True), models):
				if m != 'vit' and SR:
					continue
				mean = np.mean(metrics[p][m, d, SR], axis = (0, 1))
				std = np.std(metrics[p][m, d, SR], axis = (0, 1))
