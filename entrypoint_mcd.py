...
run_settings = [
	('baselines.isic.jax_finetune_mcd', {
			'output_dir': 'gs://ue-usrl-anuj/karm/outputs/isic/head_neck/vit_mcd/dr_<_dr>/<_seed>',
			'config': 'configs/isic/vit_finetune_mcd.py',
			'config.seed': '<_seed>',
			'config.model.transformer.attention_dropout_rate':'<_dr>',
			'config.model.transformer.dropout_rate':'<_dr>'
	},  {	'_seed': range(6),
	} ,
 {	'_dr': [0.01, 0.03, 0.1, 0.3],
	}
 ),
]
